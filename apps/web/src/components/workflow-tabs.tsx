...
) as Record<keyof typeof codeSnippets, string>;

export function WorkflowTabs() {
  const [activeTab, setActiveTab] =
    useState<keyof typeof codeSnippets>("cypress");
    ...
    <section className="border-b border-white/12">
      <div className="mx-auto max-w-[1120px] px-6 lg:px-8 py-20 sm:py-24">
        ...
        <Tabs
          ...
          className="w-full"
          onValueChange={(v) => setActiveTab(v as keyof typeof codeSnippets)}
        >
          <TabsList className="grid w-full max-w-md mx-auto grid-cols-3 bg-[#111418] border border-white/12 p-1">
            <TabsTrigger
              value="cypress"
              className="px-3 py-2.5 sm:px-4 sm:py-3 text-xs sm:text-sm text-white/70 hover:text-white data-[state=active]:bg-transparent data-[state=active]:text-white data-[state=active]:border-b-2 data-[state=active]:border-[#B8FF2C] data-[state=active]:font-semibold"
            >
              Cypress
            </TabsTrigger>
            <TabsTrigger
                ...
                <pre className="font-mono text-[12px] sm:text-[13px] text-white/80 overflow-x-auto leading-relaxed">
                  <code
                    ...
                    dangerouslySetInnerHTML={{
                      __html:
                        highlightedSnippets[
                          key as keyof typeof highlightedSnippets
                        ],
                    }}
                  />
                </pre>
