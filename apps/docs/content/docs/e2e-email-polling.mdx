...
sequenceDiagram
    ...
    participant Plop as Plop API

    Test->>App: Trigger action (login, signup)
    App->>Email: Send OTP/link email
    Email->>Plop: Deliver via inbox worker

    loop Poll until found
        Test->>Plop: GET /v1/messages/latest
        Plop-->>Test: 404 or message
    end

    Test->>Test: Extract OTP/token
    Test->>App: Submit OTP/token
```
